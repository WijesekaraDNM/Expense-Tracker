...
import { addTransaction, getCategories, updateTransaction } from "../../Services/transactionService";
...
const TransactionForm = ({selection}) => {
  ...
  const backgroundColor = formData.type === "Income" ? "bg-incomeBC" : "bg-expenseBC";
  const backgroundColor2 = formData.type === "Income" ? "bg-blurBC" : "bg-goldenHover";
  const borderColor = formData.type === "Income" ? "border-incomeBC" : "border-expenseBC";
  const textColor = formData.type === "Income" ? "text-[black]" : "text-[white]";
  ...
  useEffect(() => {
      ...
      console.log("selection name:", formData.name);
    },
    [selection,type]
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center bg-[#ababab] justify-center"> 
        ...
        <button
          ...
          <form className={`${backgroundColor2} border-[3px] w-full border-golden shadow-lg rounded-2xl m-5 text-gray-900 font-semibold text-base`}>
            <div className="flex flex-col p-5">
              <div className=" flex w-full h-72 shadow-md justify-end rounded-t-[20px] opacity-100 items-end bg-credit-card bg-cover bg-center">
                <h1 className={`flex shadow-md ${backgroundColor} ${textColor} justify-between font-bold px-5 py-3 w-full`} >
                  ...
                  <button className=" py-1  px-5 border-[2px] border-white shadow-sm hover:shadow-white hover:shadow-md rounded" onClick={Selection? handleEdit: handleSubmit}>
              ...
              <div className="flex flex-col gap-2 m-2 mt-5">
                ...
                <div className="flex relative flex-row items-center justify-start px-3">
                  <label htmlFor="date" className="block mx-1 w-[30%] my-2 ">Date</label>
                  <input
                    type="date"
                    id="date"
                    value={formData.date}
                ...
                <div className="flex relative flex-row items-center justify-start px-3">
                  <label htmlFor="amount" className="block mx-1 w-[30%] my-2 ">Amount (Rs.)</label>
                  <input
                    type="number"
                    id="amount"
                    name="amount"
                ...
                <div className="flex relative flex-row items-center justify-start px-3">
                  <label htmlFor="category" className="block mx-1 my-2 w-[30%]">Category</label>
                  <select 
                    ...
                    {formData.type === "Income"?(
                      ...
                      categories.expense.map((element, index) => (
                        <option className="focus:bg-goldenHover text-sm md:text-md" key={index} value={element}>{element}</option>
                      ))
                    )} 
                  </select>
                </div>
                <div className="flex relative flex-row items-center justify-start px-3">
                  <label htmlFor="note" className="block mx-1 my-2 w-[30%]">Note</label>
                  <textarea
                    type="textarea"
                    id="note"
                    name="note"
