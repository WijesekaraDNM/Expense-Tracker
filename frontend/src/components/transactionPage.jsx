...
const TransactionPage = ({ startingDate, endingDate, databaseUpdate, popupSelection, onMaximizeToggle, maxStatus }) => {
  const navigate = useNavigate();
  const { userId } = useAuth();
  const [state, dispatch] = useReducer(reducer, initialState);
  // const [isIncomeCardPressed, setIsIncomeCardPressed] = useState(false);
  ...
  //   console.log("toggle1:", isMaximized);
  //   onMaximizeToggle(isMaximized); // Notify HomePage about the maximize status
  //   console.log("toggle2:", isMaximized);
  // };
  // useEffect(() => {
  //   console.log("isMaxTP:", isMaximized);
  //   onMaximizeToggle(isMaximized);  // Notify the parent component with the new state
  // }, [isMaximized]);
  ...
  return (
    <>
      ...
      <div className=" h-full md:mx-5 mx-2 my-2 rounded-lg backdrop-blur-lg" style={{ backdropFilter: "blur(10px)", backgroundColor: "rgba(192, 192, 192, 0.4)", boxShadow: "0 4px 30px rgba(0, 0, 0, 0.1)", border: "1px solid rgba(255, 255, 255, 0.3)" }}>
        <div className=" w-full h-full ">
          ...
          <div  
            className={`absolute space-y-2 z-30  left-5 w-[200px] mb-2 rounded-l-lg justify-end items-center py-3 transition-all duration-500000 ease-in-out  bg-primary bg-opacity-10  shadow-lg border-[3px] border-golden border-opacity-50 ${isMenuOpen
              ? " h-auto w-48 block justify-center items-center hover:transition-transform hovet:text-opacity-100  hover:duration-50000 hover:ease-in-out text-subText hover:text-opacity-100 hover:bg-white hover:border-opacity-100 mb-10"
              : "hidden"}`}
          >
            <ul className=" flex gap-1 relative w-full !mt-[8.00px] !text-[12px] cursor-pointer px-1 ![font-family:'Inter',Helvetica]  items-start">
          ...
          <div className="hidden lg:flex flex-row justify-center items-center mx-5 text-black font-semibold">
            ...
            <Tag ref={tag1Ref} data-animation = "tag"
              className={`w-40 h-6 ${selectedTag === "All"
                ? "bg-[gray]"
                : "bg-transparent"} hover:bg-goldenHover rounded-xl text-md text-center shadow-md border-[#a2a2a2] border cursor-pointer`}
              onClick={() => handleTagClick("All")}
            >
              All
            </Tag>
            ...
            <Tag ref={tag2Ref} data-animation = "tag"
              className={`w-40 h-6 ${selectedTag === "Income"
                ? "bg-incomeBC text-white"
                : "bg-transparent"} hover:bg-incomeHover focus:bg-focusColor rounded-xl text-md border-[#a2a2a2] border text-center shadow-md cursor-pointer`}
              onClick={() => handleTagClick("Income")}
            >
              Income
              {selectedTag === "Income" &&
                <Select
                  className="flex shadow-sm w-40 z-50 text-md h-6"
                  value={selectedCategory}
                  onChange={handleIncomeCategoryChange}
                >
                  <Option value="All" className="text-md">All Incomes</Option>
                  {incomeCategories.map((category, index) =>
                    <Option key={index} value={category} className="text-md">
                      {category}
                    </Option>
                  )}
                </Select>}
            ...
            <Tag ref={tag3Ref} data-animation = "tag"
              className={` w-40 h-6 ${selectedTag === "Expense"
                ? "bg-expenseBC text-white"
                : "bg-transparent"} hover:bg-expenseHover hover:text-white rounded-xl focus:bg-focusColor border-[#a2a2a2] border text-md text-center shadow cursor-pointer`}
              onClick={() => handleTagClick("Expense")}
            >
              Expense
              {selectedTag === "Expense" &&
                <Select
                  className="flex w-40 shadow-sm h-6 text-md z-50"
                  value={selectedCategory}
                  onChange={handleExpenseCategoryChange}
                >
                  <Option value="All">All Expenses</Option>
            ...
            <div className="lg:flex hidden items-center justify-end  ">
                  <button
                    className="text-black focus:outline-none text-xl font-extrabold justify-end m-4 focus:test-gray-500  "
                    onClick={handleMaximizeToggle}
                  >
                    {
                      maxStatus?
                      <FiMinimize />:<FiMaximize/>
                    }  
                  </button>
               
            </div>
          </div>
          <div className="backdrop-blur-md md:mx-5 mx-3 mb-5 bg-white/90 p-2  rounded overflow-hidden hover:overflow-auto ">
            <div className=" py-2 ">
              <table ref={tableRef} data-animation = "table" className={`table-auto w-full ${maxStatus?'lg:h-[70vh]':'lg:h-[60vh]'} h-full rounded-2xl`}>
                <thead className="flex w-full sm:no z-50 text-[0.8rem] md:text-[1rem] text-[black] font-semibold h-12 bg-gray-expenseBC rounded-2xl ">
                  <tr key="index" className="flex w-full items-center content-center bg-gradient-to-r from-incomeBC to-expenseBC rounded-lg">
                    <th className="flex w-[35%] pl-6 border-gray-500 justify-start items-center ">
                      About Transaction
                    </th>
                    <th className="flex w-[20%] justify-start items-center pl-3 border-gray-500 ">
                ...
                <tbody className={`flex justify-center gap-1 items-center ${maxStatus?'lg:max-h-[60vh]':'lg:max-h-[60vh]'} max-h-[300px] backdrop-blur-sm w-full text-white flex-wrap cursor-pointer overflow-y-hidden hover:overflow-y-auto`}>
                  {/*{filteredItems.map((element, index) =>
                    <ExpenseCard
                      key={index}
                      selection={element}
